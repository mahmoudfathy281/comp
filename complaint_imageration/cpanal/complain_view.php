...
<?php
    ...
    if(isset($_SESSION['name'])){
        ...
        $copm = $con->prepare("SELECT complain_rec.*, 
                                    personal_record.personal_name 
                                AS c_name,
                                    entity_rec.entity_name
                                AS
                                    c_entity
                                FROM 
                                    complain_rec
                                ...
                                INNER JOIN 
                                    entity_rec
                                ON
                                    entity_rec.entity_id = complain_rec.complain_entity
                                WHERE 
                                    complain_id = ?");
        $copm->execute(array($itemid));
        $count = $copm->rowCount();
        if($count > 0){
        $info_complain = $copm->fetch();
            ...
            <div class="col-sm-12">
                <div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                    ...
                    <div id="printElement">
                        ...
                        <input type="hidden" id="complain_select" value="<?php echo $info_complain['complain_id']?>">
                                <h1 class="card-title text-center"><?php echo $info_complain['complaint_name']?>
                                </h1>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-9 thubminal" id="thubminal">
                                    <h3><span class="desc">وصف الشكوي : </span><?php echo $info_complain['complain_desc']?></h3>
                                    <ul class="list-unstyled complain">
                                        <li><i class="far fa-flag fa-fw"></i> <span class="Price">البلد المقدمة منها الشكوي </span>: <?php echo $info_complain['complain_country']?></li>
                                        <li><i class="far fa-calendar-alt fa-fw"></i> <span class="Date">طبيعة الشكوي </span>: <?php echo $info_complain['complain_nat']?></li>
                                        <li><i class="fas fa-reply fa-fw"></i> <span class="Country">موقف الشكوي </span>: <?php echo $info_complain['Complain_position']?></li>
                                        <li><i class="fas fa-paper-plane fa-fw"></i> <span class="size">الرد علي الشكوي </span>: <?php echo $info_complain['complain_riplay']?></li>
                                        <li><i class="fas fa-vote-yea fa-fw"></i> <span class="size">حالة الشكوي </span>: <?php echo $info_complain['complain_status']?></li>
                                        <li><i class="far fa-user fa-fw"></i> <span class="size">مقدم الشكوي </span>: <?php echo $info_complain['complain_user']?></li>
                                        <li><i class="fas fa-seedling fa-fw"></i> <span class="size">المجهودات </span>: <?php echo $info_complain['Efforts']?></li>
                                        <li><i class="fas fa-tag fa-fw"></i> <span class="category">جهة ورود الشكوي </span>: <a href="entity.php?entity_id=<?php echo $info_complain['complain_entity']?>"><?php echo $info_complain['c_entity']?></a></li>
                                        <li><i class="far fa-user fa-fw"></i> <span class="memmber">صاحب الشكوي </span>: <a href="profile.php?personalid=<?php echo $info_complain['complain_personal']?>"><?php echo $info_complain['c_name']?></a></li>
                                        <li><i class="far fa-tag fa-fw"></i> <span class="memmber">التاجات </span>: 
                                            <?php 
                                                $alltag = explode(",", $info_complain['tag']); 
                                    ...
                                    <div class="nav flex-column nav-pills col-md-2 attach" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                                    <div class="card card-default sidebar-menu">
                                        <div class="card-header">
                                            <i class='fas fa-file-alt fa-3x attachment'></i>
                                        </div>
                                        <div class="card-body">
                                            <a class="attachment" href='<?php echo "uplod/image_complaint/" . $info_complain['attachment'] ?>' ><?php echo $info_complain['attachment']?></a>
                                        </div>
                                    </div>
                                    <a href="save.php?complain_id=<?php echo $itemid ?>" class="btn btn-primary save"><i class="fas fa-save"></i> حفظ الشكوي</a>
...
<?php
            ...
            echo "<div class='container'><div class='alert alert-danger info'>لا يوجد شكوي بهذا الرقم</div></div>";
        }
    }
